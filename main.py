...
class YahtzeeAutoTrainer:
    ...
    def choose_action_dice(self, state):
        """Alege o selecție de zaruri bazată pe Q-Table."""
    ...
    def choose_action_category(self, state):
        """Alege o categorie bazată pe Q-Table."""
    ...
    def estimate_reward(self):
        """Estimează recompensa parțială pe baza probabilităților pentru scoruri mari."""
        counts = Counter(self.dice)
        max_count = max(counts.values())
        
        # Recompense bazate pe posibilitatea unui Yahtzee sau Full House
        if max_count == 5:
            return 50  # Yahtzee
        elif max_count == 4:
            return 25  # Posibil Four of a kind
        elif max_count == 3 and 2 in counts.values():
            return 30  # Posibil Full House
        elif self.has_straight(4):
            return 15  # Posibil Small Straight
        elif self.has_straight(5):
            return 20  # Posibil Large Straight
        else:
            return sum(self.dice) / 5  # Recompensă medie bazată pe zaruri
    ...
    def train(self, num_games=1000000, save_interval=10000, log_interval=1000):
        ...
        for game in range(1, num_games + 1):
            ...
            while self.rolls_left > 0:
                ...
                # Recompensă parțială pentru selecția zarurilor
    ...
    def save_q_tables(self, filename_dice="q_table_dice.pkl", filename_category="q_table_category.pkl"):
        """Salvează tabelele Q într-un fișier."""
